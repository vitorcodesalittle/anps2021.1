...
class ProductController @Inject()(boundary: Boundary, val controllerComponents: ControllerComponents)(implicit ec: ExecutionContext) extends BaseController with play.api.i18n.I18nSupport {
  ...
  def createProduct: Action[AnyContent] = Action {
    implicit request => {
      ...
      val productTry: Try[Product] = boundary.createProduct(productData)
      productTry match {
